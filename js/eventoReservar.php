<script>
    var arrayPrec = <?php echo json_encode($guar);?>;
    var arrayID = <?php echo json_encode($idg);?>;
    total = 0;
    bol = 0;
...
function getTotal() {
    total = 0;
    bol = 0;
    var y = document.getElementsByClassName("precios");
    var i;
    for (i = 0; i < y.length; i++) {
      pid=y[i].id;
      tmpcantbol=_int('Cantidad'+pid);
      bol+=tmpcantbol;

      total+=(parseFloat(__(pid).innerHTML)*tmpcantbol);
      
    }
    
    __('totales').innerHTML = "<h1>" + fNumber.go(total, "$") + "</h1>";
    if (bol <= 0) {
        __('tbol').classList.add('error');
    } else {
        __('tbol').classList.remove('error');
    }
}
function getPagado(){
    ctpag=0;
